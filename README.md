...
4. 刷入器等待 USB 串口枚举并检测 MicroPython；已安装则跳过。
5. 连续 3 次没有检测到 REPL 时，先执行只读 ESP32-C3 芯片识别；识别成功后才自动擦除并安装内置环境。
6. 环境可用后，刷入器会备份旧文件并上传 UFO 固件。
7. 最后会短暂以约 2% 速度测试左路，并验证 500ms 失联后双路停止。
